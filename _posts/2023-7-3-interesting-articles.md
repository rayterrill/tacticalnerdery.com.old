---
layout: post
title:  "Interesting Articles - 7/3/2023"
categories: tech
---
* [How We Scaled to 100 Million Active Users Using Kafka and Golang — Eventual Consistency]([https://www.bejarano.io/terraform-stateless/](https://itnext.io/how-we-scaled-to-100-million-active-users-using-kafka-and-golang-eventual-consistency-6241cfeba7e8))

* [GoNB, A Go Notebook Kernel for Jupyter](https://github.com/janpfeifer/gonb)

* [The way Datadog calculates percentiles at scale is very innovative](https://twitter.com/bdkozlovski/status/1675557351493890048?s=43&t=1W4M2e8Opr-93iM1M6CgPA)

* [Create Go App CLI](https://github.com/create-go-app/cli)

* [A distributed Cron service with, API, no SPOF and an easy to use dashboard.](https://dkron.io/)

* [ObjectBox Go Database - simple but powerful; frugal but fast](https://github.com/objectbox/objectbox-go)

* [the zero dependency WebAssembly runtime for Go developers](https://wazero.io/)

* [Intro.js is a lightweight JavaScript library for creating step-by-step and powerful customer onboarding tours](https://introjs.com/)

* [Case Study: Algorithmic Trading With Go](https://polygon.io/blog/case-study-algorithmict-trading-with-go/)

* [Build your own Discord/Slack bot using Supabase and OpenAI's GPT to help support teams](https://www.windmill.dev/blog/knowledge-base-discord-bot)

* [Supabase is an open source Firebase alternative.](https://supabase.com/)

* [Ultimate Go Tour](https://tour.ardanlabs.com/tour/list)

* [A language-specific implementation of OpenTelemetry in Go.](https://opentelemetry.io/docs/instrumentation/go/)

* [Advanced macOS Command-Line Tools](https://saurabhs.org/advanced-macos-commands)

* [What if your Pods need to trust self-signed certificates?](https://blog.alexellis.io/what-if-your-pods-need-to-trust-self-signed-certificates/)

* [Cloud-nomad architecture](https://octopus.com/blog/cloud-nomad-architectures)

* [Github action example using repository dispatch to kick off other workflows](https://github.com/gruntwork-io/patcher-action/blob/main/examples/github/workflows/update-dev.yml)

* [8 Terraform continuous validation use cases for AWS, Google Cloud, and Azure](https://www.hashicorp.com/blog/8-terraform-continuous-validation-use-cases-for-aws-google-cloud-and-azure)

* [Monitoring is a Pain](https://matduggan.com/were-all-doing-metrics-wrong/)

* [The Amazon Builders' Library is an hidden gem](https://twitter.com/g_bonfiglio/status/1673650452846505985?s=43&t=1W4M2e8Opr-93iM1M6CgPA)

* [ClickHouse is the fastest and most resource efficient open-source database for real-time apps and analytics.](https://clickhouse.com/)

* [VictoriaMetrics is a fast, cost-effective and scalable monitoring solution and time series database.](https://docs.victoriametrics.com/Single-server-VictoriaMetrics.html)

* [A mobile friendly web application built with Svelte for the SpaceTraders API](https://github.com/Ironolife/SpaceTraders)

* [Upload a Godot HTML5 game to itch.io with Github Actions](https://simondalvai.com/blog/godot-itchio-upload/)

* [Build beautiful cross-platform applications using Go](https://wails.io/docs/howdoesitwork/)

* [Golang wails application with svelte frontend](https://github.com/codenoid/batch-image-generator)

* [Go 1.21 will (probably) download newer toolchains on demand by default](https://utcc.utoronto.ca/~cks/space/blog/programming/Go121ToolchainDownloads)

* [Thoughts on Svelte(Kit), one year and 3 billion requests later](https://claudioholanda.ch/en/blog/svelte-kit-after-3-billion-requests/)

* [The React Graph Gallery](https://www.react-graph-gallery.com/)

* [Vault 1.14 brings ACME for PKI, AWS roles, and more improvements](https://www.hashicorp.com/blog/vault-1-14-brings-acme-for-pki-aws-roles-and-more-improvements)

* [WHAT IS THE NETWORK SPEED BETWEEN AZURE REGIONS CONNECTED WITH VIRTUAL NETWORK PEERING?](https://clusteringformeremortals.com/2018/06/29/what-is-the-network-speed-between-azure-regions-connected-with-virtual-network-peering/)

* [QUESTIONABLE ADVICE: “HOW CAN I DRIVE CHANGE AND INFLUENCE TEAMS…WITHOUT POWER?”](https://charity.wtf/2023/06/05/drive-change-and-influence-teams-without-power/)

* [Running collections on the command line with Newman](https://learning.postman.com/docs/collections/using-newman-cli/command-line-integration-with-newman/)

* [How we learned to improve Kubernetes CronJobs at Scale (Part 1 of 2)](https://eng.lyft.com/improving-kubernetes-cronjobs-at-scale-part-1-cf1479df98d4)

* [mTLS in 5… 10… Okay 20 Minutes](https://itnext.io/mtls-in-5-10-okay-20-minutes-6602eddae6fe)

* [Setup a high availability kubernetes raspberry pi cluster](https://tissen.dev/posts/production-like-kubernetes-on-raspberrypi-setup)

* [Leaking secrets through caching with Bunny CDN](https://httptoolkit.com/blog/bunny-cdn-caching-vulnerability/)

* [https://medium.com/swlh/how-to-host-your-angular-reactjs-vuejs-spa-on-github-pages-2d9ab102ac7b](https://medium.com/swlh/how-to-host-your-angular-reactjs-vuejs-spa-on-github-pages-2d9ab102ac7b)

* [Gopher Wrangling. Effective error handling in Go](https://stephenn.com/2023/06/gopher-wrangling.-effective-error-handling-in-go/)

* [Building your static website with Svelte, SvelteKit and TailwindCSS](https://dev.to/robertobutti/how-to-start-building-your-static-website-with-svelte-and-tailwindcss-hbk)

* [{JSON} Placeholder](https://jsonplaceholder.typicode.com/)

* [Storybook is a frontend workshop for building UI components and pages in isolation.](https://storybook.js.org/)

* [iOS 17 automatically removes tracking parameters from links you click on](https://9to5mac.com/2023/06/08/ios-17-link-tracking-protection/)

* [Introducing the Postman VS Code extension](https://blog.postman.com/introducing-the-postman-vs-code-extension/)

* [This repo shows how to run tailscale on fly, specifically to run exit nodes.](https://github.com/patte/fly-tailscale-exit)

* [How to start a Go project in 2023](https://boyter.org/posts/how-to-start-go-project-2023/)

* [50 Shades of Go: Traps, Gotchas, and Common Mistakes for New Golang Devs](http://devs.cloudimmunity.com/gotchas-and-common-mistakes-in-go-golang/)

* [Implementing a distributed key-value store on top of implementing Raft in Go](https://notes.eatonphil.com/2023-05-25-raft.html)

* [Terraform 1.5 brings config-driven import and checks](https://www.hashicorp.com/blog/terraform-1-5-brings-config-driven-import-and-checks)

* [Slack uses Apache Kafka at scale](https://twitter.com/bdkozlovski/status/1661700334157430784?s=43&t=1W4M2e8Opr-93iM1M6CgPA)

* [Wireguard on Azure](https://prabeshthapa.medium.com/wireguard-on-azure-4a625bf350bf)

* [A collection of modern, server-safe React hooks – from the ui.dev team](https://usehooks.com/)

* [A list of engineering leadership resource links.](https://github.com/ryanburgess/engineer-manager)

* [Data Compression Drives the Internet. Here’s How It Works.](https://www.quantamagazine.org/how-lossless-data-compression-works-20230531/)

* [Explore pitch](https://learningmusic.ableton.com/notes-and-scales/notes-and-scales.html)

* [Terraform check{} Block](https://unfriendlygrinch.info/posts/terraform-check-block/?mkt_tok=ODQ1LVpMRi0xOTEAAAGLzdHtiMUR6KKpIhKZTLI5tQYRAPWAsAWNl2RqjIv575T4KqK4gJDQALg4fX_rTgmP9-kIBlZLVgoYtoA2J533M6_Ch6bgBMpntx8F0NhDho82aA)

* [D1: We turned it up to 11](https://blog.cloudflare.com/d1-turning-it-up-to-11/)

* [Growing from engineer to manager and thriving](https://newsletter.eng-leadership.com/p/growing-from-engineer-to-manager)

* [OpenObserve is a cloud native observability platform built specifically for logs, metrics, traces and analytics designed to work at petabyte scale.](https://github.com/openobserve/openobserve)

* [Using Kubernetes Service for Proxying to External Services](https://kristhecodingunicorn.com/post/k8s_proxy_svc/)

* [Connect to your instances without requiring a public IPv4 address using EC2 Instance Connect Endpoint](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/Connect-using-EC2-Instance-Connect-Endpoint.html)

* [Vault Secrets Operator for Kubernetes now GA](https://www.hashicorp.com/blog/vault-secrets-operator-for-kubernetes-now-ga)

* [Weave Policy Engine is now Open Source](https://www.weave.works/blog/weave-policy-engine-is-now-open-source)

* [Announcing Live Tail in Amazon CloudWatch Logs, providing real-time exploration of logs](https://aws.amazon.com/about-aws/whats-new/2023/06/live-tail-amazon-cloudwatch-logs/)

* [AWS Copilot announces Static Site pattern to host single-page web applications](https://aws.amazon.com/about-aws/whats-new/2023/05/aws-copilot-static-site-pattern-single-page-web-applications/)

* [https://fleet.rancher.io/](https://fleet.rancher.io/)

* [Using eBPF in Kubernetes](https://kubernetes.io/blog/2017/12/using-ebpf-in-kubernetes/)

* [Announcing Hurl 4.0.0](https://hurl.dev/blog/2023/06/30/announcing-hurl-4.0.0.html)
